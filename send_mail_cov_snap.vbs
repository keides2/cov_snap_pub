...
Dim strStream, strSnapshotID, strZipFilePath, strZipFileName, strSenderEmailAddress, strErrorlevel
...
' 開始
'------------------------------------
Wscript.Echo vbCrLf & "■ send_mail_cov_snap.vbs 開始"
' yyyymmdd 形式で現在日付を取得
...
' ログのファイル名
LOG_NAME = "send_mail_cov_snap-" & strFormattedDateTime & ".log"
' ログレベル(INFO)
Const LOG_INFO = "INFO"
' ログレベル(ERROR)
...
' ログ出力開始
Call WriteLog("■ send_mail_cov_snap.vbs ログ出力開始", LOG_INFO)
...
' ストリーム名、スナップショットID、、メール送信者名、エラーレベル、添付するZIPファイルパス
' %1: %STREAM%, %2: %SNAPSHOT1%, %3: %SENDEREMAILADDRESS%, %4: %error_value%, %5: %ZIP_FILE_PATH%
' 添付ファイルは絶対パス指定のこと（相対パスでは添付されない）
strStream = oParam(0)
strSnapshotID = oParam(1)
strSenderEmailAddress = oParam(2)
strErrorlevel = oParam(3)
strZipFilePath = oParam(4)
...
' アドレス・ファイルの読み込み
'------------------------------------
' strAddressFile = strAddressDir & strGroup & "_address.csv"
